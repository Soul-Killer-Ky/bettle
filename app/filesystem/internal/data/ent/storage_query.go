...
func (sq *StorageQuery) QueryQuotes() *QuoteQuery {
	query := (&QuoteClient{config: sq.config}).Query()
...
func (sq *StorageQuery) Clone() *StorageQuery {
	...
	return &StorageQuery{
		...
		withQuotes: sq.withQuotes.Clone(),
...
func (sq *StorageQuery) WithQuotes(opts ...func(*QuoteQuery)) *StorageQuery {
	query := (&QuoteClient{config: sq.config}).Query()
...
func (sq *StorageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Storage, error) {
	...
	if query := sq.withQuotes; query != nil {
		if err := sq.loadQuotes(ctx, query, nodes,
			func(n *Storage) { n.Edges.Quotes = []*Quote{} },
			func(n *Storage, e *Quote) { n.Edges.Quotes = append(n.Edges.Quotes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StorageQuery) loadQuotes(ctx context.Context, query *QuoteQuery, nodes []*Storage, init func(*Storage), assign func(*Storage, *Quote)) error {
	...
	query.Where(predicate.Quote(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(storage.QuotesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.storage_quotes
		if fk == nil {
			return fmt.Errorf(`foreign-key "storage_quotes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "storage_quotes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
