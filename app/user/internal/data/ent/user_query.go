...
func (uq *UserQuery) QueryCreatedGroups() *GroupQuery {
	query := (&GroupClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) QueryGroupMembers() *GroupMemberQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCreatedGroups: uq.withCreatedGroups.Clone(),
		withGroupMembers:  uq.withGroupMembers.Clone(),
...
func (uq *UserQuery) WithCreatedGroups(opts ...func(*GroupQuery)) *UserQuery {
	query := (&GroupClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withCreatedGroups = query
	return uq
}
...
func (uq *UserQuery) WithGroupMembers(opts ...func(*GroupMemberQuery)) *UserQuery {
	query := (&GroupMemberClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withGroupMembers = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			uq.withCreatedGroups != nil,
			uq.withGroupMembers != nil,
		}
	)
	...
	if query := uq.withJoinedGroups; query != nil {
		if err := uq.loadJoinedGroups(ctx, query, nodes,
			func(n *User) { n.Edges.JoinedGroups = []*Group{} },
			func(n *User, e *Group) { n.Edges.JoinedGroups = append(n.Edges.JoinedGroups, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withCreatedGroups; query != nil {
		if err := uq.loadCreatedGroups(ctx, query, nodes,
			func(n *User) { n.Edges.CreatedGroups = []*Group{} },
			func(n *User, e *Group) { n.Edges.CreatedGroups = append(n.Edges.CreatedGroups, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withGroupMembers; query != nil {
		if err := uq.loadGroupMembers(ctx, query, nodes,
			func(n *User) { n.Edges.GroupMembers = []*GroupMember{} },
			func(n *User, e *GroupMember) { n.Edges.GroupMembers = append(n.Edges.GroupMembers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadJoinedGroups(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.JoinedGroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(user.JoinedGroupsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.JoinedGroupsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.JoinedGroupsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "joined_groups" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadCreatedGroups(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	...
	for _, n := range neighbors {
		fk := n.user_created_groups
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_created_groups" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_created_groups" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadGroupMembers(ctx context.Context, query *GroupMemberQuery, nodes []*User, init func(*User), assign func(*User, *GroupMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.GroupMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.GroupMembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
